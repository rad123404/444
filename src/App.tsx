...
export default function App() {
  ...
  useEffect(() => {
    const unsubSchedules = subscribeToDoc<ScheduleProfiles>('schedules', data => {
      if (data && typeof data === 'object') setSchedules(data);
    });
    const unsubHomework = subscribeToDoc<HomeworkStore>('homework', data => {
      if (data && typeof data === 'object') setHomework(data);
    });
    const unsubDuties = subscribeToDoc<DutiesStore>('duties', data => {
      if (data && typeof data === 'object') setDuties(data);
    });
    const unsubBirthdays = subscribeToDoc<BirthdayItem[]>('birthdays', data => {
      if (Array.isArray(data)) setBirthdays(data);
    });
    const unsubEvents = subscribeToDoc<ClassEvent[]>('events', data => {
      if (Array.isArray(data)) setEvents(data);
    });
    const unsubPoll = subscribeToDoc<PollData>('currentPoll', data => {
      if (data && typeof data === 'object') setCurrentPoll(data);
    });
    const unsubPollHistory = subscribeToDoc<PollData[]>('pollHistory', data => {
      if (Array.isArray(data)) setPollHistory(data);
    });
    const unsubIsPollActive = subscribeToDoc<boolean>('isPollActive', data => {
      if (typeof data === 'boolean') setIsPollActive(data);
    });
